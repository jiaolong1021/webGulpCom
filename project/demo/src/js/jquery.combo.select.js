...
(function (factory) {
	...
	$.extend(Plugin.prototype, {
		...
		_keyup: function(event){
			...
			var canseelisizes = this.$el.parent().find('li:visible').size();
			if (canseelisizes <= 0) {
			    this.$el.parent().find('ul').css({ 'height': '0', 'min-width': '94%' });
			} else {
			    this.$el.parent().find('ul').css({ 'height': 'auto', 'min-width': '100%' });
			}
		},
